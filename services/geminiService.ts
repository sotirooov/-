...
export const generateFeedback = async (scenario: Scenario, userAnswer: string): Promise<string> => {
  ...
  const prompt = `Потребителят беше представен със следния сценарий:
Тяло: ${JSON.stringify(scenario.body)}
Въпрос: ${scenario.question}
Правилен отговор: ${correctAnswerText}
Отговор на потребителя: ${userAnswer}

Обясни защо отговорът на потребителя е правилен или грешен. Бъди изключително кратък и ясен (1-2 изречения). Фокусирай се върху основния учебен момент.`;
